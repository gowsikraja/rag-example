...
def load_all_documents(data_dir: str) -> List[Any]:
    ...
    print(
        f"[DEBUG] Found {len(txt_files)} TXT files: {[str(f) for f in txt_files]}")
    ...
    print(
        f"[DEBUG] Found {len(csv_files)} CSV files: {[str(f) for f in csv_files]}")
    ...
    print(
        f"[DEBUG] Found {len(docx_files)} Word files: {[str(f) for f in docx_files]}")
    ...
    for json_file in json_files:
        ...
        try:
            loader = JSONLoader(str(json_file), '.quiz')
            loaded = loader.load()
